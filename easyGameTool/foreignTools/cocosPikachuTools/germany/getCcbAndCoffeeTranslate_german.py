...
def makeCcbTranslate():
    ...
    # SQL 查询语句
    sql = "SELECT Id,German from ccbTranslate WHERE Id IS NOT NULL and Id != 0 and German is not NULL"
    try:
        # 执行SQL语句
        cursor.execute(sql)
        # 获取所有记录列表
...
def makeCoffeeTranslate():
    ...
    # SQL 查询语句
    sql = "SELECT Id,German from coffeeTranslate6 WHERE Id IS NOT NULL and Id != 0 and German is not NULL"
    try:
        # 执行SQL语句
        cursor.execute(sql)
        # 获取所有记录列表

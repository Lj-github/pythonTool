...
import shutil
import binascii
ip = "http://192.168.0.9/update/majhong/assets_bj//"
...
def createProject(packageUrl,remoteVersionUrl,remoteManifestUrl,version,engineVersion,appV,assets = []):
    ...
    if assets.__len__()>0:
        for file in assets:
            md = {}
            print file
            print  generate_file_md5value("../assets/"+ file)
...
def init(flag): #是否为热跟新
    ...
    # luac = 'cocos luacompile -s ' + '"' + cpath + '\\update\\src"' + ' -d "' + cpath + '\\assets\\src"'
    # print copy
    # print luac
    # os.system(copy)
    # os.system(luac)
    ...
    r = file("project.manifest", "w")
    r.write(json.dumps(createProject(ip+dataStr, ip+"version.manifest",ip+"project.manifest",dataStr, "Cocos2d-x v3.10",appVersion, allFile)) )
    r.close()
    v = file('version.manifest','w')
    v.write(json.dumps(createVersion(ip+dataStr, ip+"version.manifest",ip+"project.manifest",dataStr,  "Cocos2d-x v3.10",appVersion)))
    v.close()

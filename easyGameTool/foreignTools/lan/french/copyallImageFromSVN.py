...
def replaceOneFile(originText, newText, path):
    ...
    with open(path,'w') as w:
        if lines.find("<string>Tap to get</string>") and originText == "<string>Tap to get</string>" :
            print("发现")

        newStr = lines.replace(originText,newText)
        w.write(newStr)
    ...
    for file in fileArr:
        ...
        if file.endswith(".ccb"):
            ...
            if item[2] == '' or item[2] == None:
                continue
            replaceOneFile('<string>' + item[0] + '</string>', '<string>' + item[2] + '</string>', path)
            #可能之前的语言

...
def makeCcbTranslate():
    ...
    # SQL 查询语句
    sql = "SELECT Id,German from ccbTranslate_copy1 WHERE Id IS NOT NULL and Id != 0 and German is not NULL"
    try:
        # 执行SQL语句
        cursor.execute(sql)
        # 获取所有记录列表
...
def makeCoffeeTranslate():
    ...
    # SQL 查询语句
    sql = "SELECT Id,German from coffeeTranslate WHERE Id IS NOT NULL and Id != 0 and German is not NULL"
    try:
        # 执行SQL语句
        cursor.execute(sql)
        # 获取所有记录列表
